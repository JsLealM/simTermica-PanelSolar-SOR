...
i, j = pos_max
es_frontera = (i == 0) or (i == N-1) or (j == 0) or (j == N-1)
...
print(f'  Abajo (fila {N-1}): min={T_inicial[N-1, :].min():.2f}, max={T_inicial[N-1, :].max():.2f}')
print(f'  Izquierda (col 0): min={T_inicial[:, 0].min():.2f}, max={T_inicial[:, 0].max():.2f}')
print(f'  Derecha (col {N-1}): min={T_inicial[:, N-1].min():.2f}, max={T_inicial[:, N-1].max():.2f}')
